...
export function buildSignatureInput(p: SignParams): string {
...
export function buildSignatureBase(signatureInput: string, authority: string, path: string): string {
...
export function signEd25519(signatureBase: string, seed32Bytes: Uint8Array): string {
...
export function signRsaPssSha256(signatureBase: string, privateKeyPem: string): string {
...
export function signTap(p: SignParams, secret: { ed25519SeedB64?: string; rsaPrivateKeyPem?: string }): { signature_input: string; signature: string; commitment: string } {
  ...
  if (p.alg === 'ed25519') {
    ...
    if (seed.length !== 32) throw new Error('ED25519 seed must be 32 bytes base64');
