import { signTap, TapAlg } from './sign.js';

export type TapRouterOptions = {
  basePath?: string; // default '/tap'
  keyId: string;
  alg: TapAlg;
  ed25519PublicKeyB64?: string;
  ed25519PrivateSeedB64?: string; // 32-byte seed in base64
  rsaPublicKeyPem?: string;
  rsaPrivateKeyPem?: string;
};

export function registerTapRoutes(app: any, opts: TapRouterOptions) {
  const base = opts.basePath || '/tap';

  app.get(`${base}/keys/:keyId`, (req: any, res: any) => {
    try {
      ...
      let public_key = '';
      if (alg === 'ed25519') public_key = opts.ed25519PublicKeyB64 || '';
      else if (alg === 'rsa-pss-sha256') public_key = (opts.rsaPublicKeyPem || '').replace(/\n/g, '\n');
      else {
  ...
  app.post(`${base}/sign`, (req: any, res: any) => {
    try {
      ...
      const algorithm = String(alg || opts.alg || '').toLowerCase() as TapAlg;
      ...
      const { signature_input, signature, commitment } = signTap({ authority, path, keyId: effectiveKeyId, alg: algorithm, ttlSec, tag, nonce }, secret as any);
