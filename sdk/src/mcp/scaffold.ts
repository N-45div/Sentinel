...
export type XmcpScaffoldOptions = {
  outDir: string;
  projectName?: string;
  includeOnchain?: boolean;
};

function pkgJson(options: XmcpScaffoldOptions): string {
  const base = {
    name: options.projectName || 'mcp-x402-app',
    version: '0.1.0',
    description: 'XMCP project scaffolded by @divij_web3dev/sentinel-sdk',
    engines: { node: '>=20.0.0' },
    type: 'commonjs',
    scripts: {
      build: 'xmcp build',
...
const config: XmcpConfig = {
  ...
  webpack: (cfg) => {
    // Ensure the generated .xmcp/import-map.js is parsed as ESM
    const anyCfg = cfg as any;
    const moduleCfg = (anyCfg.module ||= { rules: [] });
    moduleCfg.rules ||= [];
    moduleCfg.rules.push({
      test: /import-map\\.js$/,
      type: "javascript/esm",
...
const readmeMd = `# XMCP app (generated by @divij_web3dev/sentinel-sdk)\n\n- Scripts:\n  - dev: xmcp dev\n  - build: xmcp build\n  - start: node dist/http.js\n\n- Tools:\n  - sentinel.create_job\n  - sentinel.checkpoint\n  - sentinel.settle\n\nWire your x402 server MCP_URL to this HTTP service to use paid tools with commitments.\n`;

async function ensureDir(p: string) {
  await fs.mkdir(p, { recursive: true });
}

async function writeFileIfMissing(p: string, content: string) {
  try {
    await fs.access(p);
...
export async function scaffoldXmcpProject(options: XmcpScaffoldOptions): Promise<void> {
  ...
  if (options.includeOnchain) await ensureDir(srcTypes);
  ...
  // Suggest Node 20 LTS for xmcp dev compatibility
  await writeFileIfMissing(path.join(root, '.nvmrc'), '20\n');
  ...
  const toolFirecrawlScrape = `import { z } from "zod";
import type { ToolMetadata } from "xmcp";

export const schema = {
  url: z.string().url().describe("URL to scrape"),
  mode: z.enum(["markdown","html"]).default("markdown").describe("Output format"),
};

export const metadata: ToolMetadata = {
  name: "firecrawl.scrape",
  description: "Scrape a web page using Firecrawl API and return content",
};

const Args = z.object(schema);

export default async function handler(input: unknown) {
  const { url, mode } = Args.parse(input);
  ...
  const structured = { success: r.ok, mode, url, data } as const;
  const text = r.ok ? ('Scraped ' + url + ' (' + mode + ')') : ('Scrape failed ' + url + ': ' + (data?.error || r.status));
  return { content: [{ type: "text", text }], structuredContent: structured } as any;
}
`;
  await writeFileIfMissing(path.join(srcTools, 'firecrawl-scrape.ts'), toolFirecrawlScrape);
