...
function clientSolanaPackageJson() {
    return JSON.stringify({
        ...
        dependencies: { dotenv: '^16.4.5', '@solana/web3.js': '^1.95.3' },
...
function readKeypair(): Keypair {
  ...
  const j = process.env.SOLANA_PRIVATE_KEY_JSON || '';
  if (j) {
  ...
  throw new Error('Provide SOLANA_KEYPAIR_PATH or SOLANA_PRIVATE_KEY_JSON');
}

function readBody(): any {
  const raw = process.env.JSON_BODY || '';
  if (raw) {
    try { return JSON.parse(raw); } catch {}
  }
  const p = process.env.ENDPOINT_PATH || '/research';
  if (p === '/mcp/execute') {
    return { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'firecrawl.scrape', arguments: { url: 'https://example.com', mode: 'markdown' } } };
  }
  return { query: 'hello' };
}

async function signHeaders(method: string, url: string, headers: Record<string,string>, body: string) {
  const signer = process.env.TAP_SIGN_URL || '';
  if (!signer) return headers;
  try {
    const r = await fetch(signer, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ method, url, headers, body }) });
    const data: any = await r.json().catch(() => ({}));
    const hs = data?.headers;
    if (hs && typeof hs === 'object') {
      return { ...headers, ...hs } as Record<string,string>;
    }
  } catch {}
  return headers;
}
...
async function main() {
  ...
  const path = process.env.ENDPOINT_PATH || '/research';
  const url = base + path;
  if (!base) throw new Error('RESOURCE_SERVER_URL required');

  const bodyInit = JSON.stringify(readBody());
  let headers1: Record<string,string> = { 'Content-Type': 'application/json' };
  headers1 = await signHeaders('POST', url, headers1, bodyInit);
  const r1 = await fetch(url, { method: 'POST', headers: headers1, body: bodyInit });
  if (r1.status !== 402) {
    const txt = await r1.text();
    console.log('[client-solana] status:', r1.status, txt);
    return;
  }
  const pr = r1.headers.get('x-payment-request') || '';
  console.log('[client-solana] payment-request:', pr);
  let payload: any;
  try { payload = JSON.parse(pr); } catch { try { payload = JSON.parse(Buffer.from(pr, 'base64').toString('utf-8')); } catch {} }
  if (!payload) throw new Error('Cannot parse x-payment-request');

  const recipient = new PublicKey(payload?.recipient);
  const amount = BigInt(payload?.amount || '0');
  const kp = readKeypair();
  const conn = new Connection('https://api.devnet.solana.com');
  const tx = new Transaction().add(SystemProgram.transfer({ fromPubkey: kp.publicKey, toPubkey: recipient, lamports: Number(amount) }));
  const sig = await sendAndConfirmTransaction(conn, tx, [kp]);
  console.log('[client-solana] payment sig:', sig);

  let headers2: Record<string,string> = { 'Content-Type': 'application/json', 'x-payment': sig } as any;
  headers2 = await signHeaders('POST', url, headers2, bodyInit);
  const r2 = await fetch(url, { method: 'POST', headers: headers2, body: bodyInit });
  const body2 = await r2.text();
  console.log('[client-solana] final status:', r2.status);
  console.log(body2);
}
...
// client-fetch (x402-fetch starter-style)
...
function clientInspectPackageJson() {
    return JSON.stringify({
        name: 'client-inspect',
        version: '0.1.0',
        private: true,
...
function clientInspectEnvExample() {
    return [
        '# Merchant server URL',
        'RESOURCE_SERVER_URL=http://localhost:4021',
        'ENDPOINT_PATH=/research',
...
async function main() {
  ...
  if (!baseURL) throw new Error('Set RESOURCE_SERVER_URL in .env');

  const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: 'hello' }) });
  console.log('[client-inspect] status:', r.status);
  console.log('[client-inspect] headers:');
  r.headers.forEach((v, k) => console.log('  ' + k + ': ' + v));
  try {
    const body = await r.json();
    console.log('[client-inspect] json:', body);
  } catch {
    const text = await r.text();
    console.log('[client-inspect] text:', text.slice(0, 500));
  }
}
...
function landingIndexHtml(projectName) {
...
async function main() {
    ...
    // client-inspect (simple fetch-based inspector, chain-agnostic; works with solana-devnet merchant)
    const inspectDir = path.join(outDir, 'client-inspect');
    await write(path.join(inspectDir, 'package.json'), clientInspectPackageJson());
    await write(path.join(inspectDir, '.env.example'), clientInspectEnvExample());
    await write(path.join(inspectDir, 'src', 'index.ts'), clientInspectIndexTs());
